...
class NewwbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('newwb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `newwb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM newwb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO newwb (date, title, link, content)"
